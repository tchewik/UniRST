...
class RS3ForestSplitter:
    ...
    def __call__(self, filename: str, output_dir: str, remove_restatement=False):
        ...
        for event, elem in context:
            if elem.tag == 'header':
                ...
                if remove_restatement:
                    #### For some reason, relation Restatement is duplicating in the rus.rst.rrt markup
                ...
                with open(os.path.join(output_dir, f'{output_filename}_part_{i}.rs3'), 'w') as f:
                    ...
                    for element in trees_body.get(tree_number):
                        ...
                        if element.tag == 'segment':
                            f.write(f'\t\t<{element.tag} id="{_id}" {parent} relname="{_relname}"')
