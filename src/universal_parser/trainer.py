...
from src.universal_parser.data_manager import DataManager
from src.universal_parser.src.config_reader import ConfigReader
from src.universal_parser.src.parser.data import Data
from src.universal_parser.src.parser.parsing_net import ParsingNet
from src.universal_parser.src.parser.parsing_net_bottom_up import ParsingNetBottomUp
from src.universal_parser.src.parser.training_manager import TrainingManager
...
class Trainer:
    def __init__(self,
                 ...
                 model__segmenter__separated=True,
                 ...
                 model__parser_type='top-down',
                 model__use_rel_weights=True,
                 model__use_union_relations=False,
        ...
        for corpus_name in data__corpora:
            ...
            if not os.path.isfile(data_manager_path):
                if not os.path.isdir('data/dms'):
                    os.mkdir('data/dms')

                dp = DataManager(corpus=corpus_name)
                dp.from_rs3()
                dp.save(data_manager_path)

            self.data_managers.append(DataManager(corpus=corpus_name, aug=data__data_augmentation).from_pickle(data_manager_path))
        ...
        self.model_segmenter_separated = model__segmenter__separated
        ...
        self.model_parser_type = model__parser_type
        self.model_use_rel_weights = model__use_rel_weights
        self.model_use_union_relations = model__use_union_relations
        ...
        self.trainer_tracker = trainer__tracker
    ...
    def _load_data(self):
        ...
        for data_manager in self.data_managers:
            train, val, test = data_manager.get_data()
    ...
    def _get_label_weights(self, relation_labels, num_relations):
        ...
        weights = (1.01 - normalize(count)).to(self._cuda_device)

        return weights

    def _prepare_unique_relation_inventories(self, relation_tables, label_weights):
        """Merge identical relation tables and compute mapping."""
        unique_tables = []
        unique_weights = []
        mapping = []

        for table, w in zip(relation_tables, label_weights):
        ...
        return unique_tables, unique_weights, mapping

    def _prepare_union_relation_inventories(self, relation_tables, label_weights):
        """Create a union label inventory and dataset-specific masks."""
        union_table = []
        label2id = {}
        ...
        for table, weights in zip(relation_tables, label_weights):
            mask = [False] * len(union_table)
            w_full = [0.0] * len(union_table) if weights is not None else None
            mapping = []
            for idx, lbl in enumerate(table):
                uid = label2id[lbl]
                mask[uid] = True
                if weights is not None:
                    w_full[uid] = float(weights[idx])
                mapping.append(uid)
            dataset_masks.append(mask)
            union_weights.append(torch.tensor(w_full, device=self._cuda_device) if w_full is not None else None)
            label_maps.append(mapping)

        return union_table, union_weights, dataset_masks, label_maps

    def _prepare_relation_inventories(self, relation_tables, label_weights):
        if self.model_use_union_relations and len(relation_tables) > 1:
            return self._prepare_union_relation_inventories(relation_tables, label_weights)
        else:
            return self._prepare_unique_relation_inventories(relation_tables, label_weights)
    ...
    def _setup_model(self):
        ...
        for name, param in self._transformer.named_parameters():
            ...
            layer_num = re.findall(r"layer\.(\d+)\.", name)
        ...
        model_cls = ParsingNet if self.model_parser_type == 'top-down' else ParsingNetBottomUp

        rel_tables = [dm.relation_table for dm in self.data_managers]
        prep = self._prepare_relation_inventories(rel_tables, self._label_weights)
        if self.model_use_union_relations and len(rel_tables) > 1:
            union_table, union_weights, dataset_masks, label_maps = prep

            def _remap(data, mapping):
                if data is None:
                    return None
                data.relation_label = [[mapping[idx] for idx in doc] for doc in data.relation_label]
                return data

            for i, mapping_ in enumerate(label_maps):
                self.data_train[i] = _remap(self.data_train[i], mapping_)
                if i < len(self.data_val):
                    self.data_val[i] = _remap(self.data_val[i], mapping_)
                if i < len(self.data_test):
                    self.data_test[i] = _remap(self.data_test[i], mapping_)

            mapping = list(range(len(rel_tables)))
            relation_vocab = union_table
            classes_numbers = [len(union_table)]
            label_weights = union_weights
        else:
            unique_tables, unique_weights, mapping = prep
            relation_vocab = None
            dataset_masks = None
            classes_numbers = [len(t) for t in unique_tables]
            label_weights = unique_weights
            rel_tables = unique_tables

        self.model = model_cls(relation_tables=rel_tables,
                                relation_vocab=relation_vocab,
                                dataset_masks=dataset_masks if self.model_use_union_relations and len(self.data_managers) > 1 else None,
                                ...
                                separated_segmentation=self.model_segmenter_separated,
