...
class CustomTokenizer:
    """Tokenizer that first splits on words, then merges specified MWEs."""

    def __init__(self):
        # Use empty-string separator so MWEs stay glued together
        self.mwe_tokenizer = nltk.tokenize.MWETokenizer(separator="")
        # Add the end-of-paragraph marker used in RST-DT
        self.mwe_tokenizer.add_mwe(("<", "P", ">"))

    def tokenize(self, text):
        """Tokenize `text` into words and then apply MWE merging."""
        words = nltk.word_tokenize(text)
        return self.mwe_tokenizer.tokenize(words)
...
def parse_xml(rs3_file):
    """
    Parse an RS3 XML file, trying multiple encodings.

    Tries UTF-8 and Windows-1252 first; if those fail, falls back to the
    parser’s default encoding. Exits the program on irrecoverable failure.

    :param rs3_file: path to the .rs3 file
    :return: (root_element, parsed_tree) or exits on failure
    """
    encodings = ("utf-8", "windows-1252")
    for encoding in encodings:
        try:
            parser = etree.XMLParser(encoding=encoding)
            xml_tree = etree.parse(rs3_file, parser)
            return xml_tree.getroot(), xml_tree
        except (etree.XMLSyntaxError, UnicodeDecodeError):
            # try the next encoding
            continue

    # final fallback: default parser
    try:
        parser = etree.XMLParser()
        xml_tree = etree.parse(rs3_file, parser)
        return xml_tree.getroot(), xml_tree
    except etree.XMLSyntaxError:
        sys.exit(f"Unable to read file: {rs3_file}")


def get_relations_type(rs3_tree):
    """
    Extract all relation names and their types from the <relations> section
    of an RS3 XML tree.

    :param rs3_tree: an lxml ElementTree or Element
    :return: dict mapping relation-name to list of types
    """
    relations = {}
    for rel in rs3_tree.iterfind('.//header/relations/rel'):
        name = rel.attrib.get('name', '').replace(' ', '-')
        rel_type = rel.attrib.get('type')
        if not rel_type:
            # skip entries without an explicit type
            continue

        # accumulate types in a set to avoid duplicates
        relations.setdefault(name, set()).add(rel_type)
...
def writeEdus(doc, pathout):
    ...
    """
    edufile = os.path.join(pathout, os.path.basename(doc.path)[:-4] + ".edus")
