...
class TrainingManager:
    ...
    def __init__(self, model, train_data, dev_data, test_data,
                 ...
                 project=None, run_name=None, config=None, tracker='wandb'):
        """
        ...
        if tracker == 'wandb':
            ...
            if Task is None:
                raise ImportError('clearml is required for tracker "clearml"')
            self.run = Task.init(project_name=project, task_name=run_name)
            if config is not None:
                self.run.connect(config)
            self.logger = self.run.get_logger()
        else:
            self.run = None
            self.logger = None
            run_name = run_name or 'tmp'
        ...
        self._cuda_cache_dump_frequency = 0.01

    def _log(self, metrics: dict):
        """Log metrics using the selected tracker."""
        if self.tracker == 'wandb' and self.logger is not None:
            self.logger.log(metrics)
        elif self.tracker == 'clearml' and self.logger is not None:
            step = metrics.get('step', 0)
            for k, v in metrics.items():
                if k == 'step':
                    continue
                try:
                    self.logger.report_scalar(title=k, series='scalar', value=float(v), iteration=step)
                except Exception:
                    pass
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            # log metrics
            self._log(metrics_all)

            if metrics_all['gs_test_f1_span'] == 0:
                shutil.rmtree(self.save_dir)
                raise RuntimeError('Zero metrics. Stopping the loop.')

            # save best model
            if metrics_all['e2e_val_f1_rel'] > best_metrics['e2e_val_f1_rel']:
    ...
    def _final_loss(self, loss_tree_batch, loss_label_batch, loss_segment_batch,
        ...
        if self.use_dwa_loss:
            ...
            if len(label_loss_iter_list) >= 2 * self.dwa_bs:
                ...
                return w_tree * loss_tree_batch + w_label * loss_label_batch + w_edu * loss_segment_batch
