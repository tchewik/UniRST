...
from src.universal_parser.predictor import Predictor
from src.universal_parser.src.parser.metrics import get_batch_metrics
from src.universal_parser.src.parser.metrics import get_micro_metrics
# from src.corpus_conversion.corpus_stats import select_genre
...
class EvalDMRST:
    ...
    def __init__(self, model_path, corpus, dataset_index=0, cuda_device=0):
        """
        ...
        self.predictor = Predictor(model_path, cuda_device=cuda_device, relinventory_idx=self.dataset_index)

        dm_file = f'data/dms/data_manager_{corpus.lower()}.pickle'
        dm = DataManager(corpus=corpus).from_pickle(dm_file)
        self.data = dm.get_data()[-1]  # test data
            ...
            with torch.no_grad():
                loss_tree_batch, loss_label_batch, \
                    span_batch, label_tuple_batch, predict_edu_breaks = self.predictor.model.testing_loss(
                        input_sentence=batch.input_sentences,
                        input_sent_breaks=batch.sent_breaks,
                        input_entity_ids=batch.entity_ids,
                        input_entity_position_ids=batch.entity_position_ids,
                        input_edu_breaks=batch.edu_breaks,
                        label_index=batch.relation_label,
                        parsing_index=batch.parsing_breaks,
                        generate_tree=True,
                        use_pred_segmentation=use_pred_segmentation,
                        dataset_index=[self.dataset_index] * len(batch.input_sentences)
                )
