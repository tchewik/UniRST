...
class EvalUniversal:
    ...
    def __init__(self, model_path, corpus, cuda_device=0, relinventory_idx=0):
        """
        ...
        self.predictor.relinventory_idx = relinventory_idx
        print(f'{self.predictor.relinventory_idx = }')

        dm_file = f'data/dms/data_manager_{corpus.lower()}.pickle'
        dm = DataManager(corpus=corpus).from_pickle(dm_file)

        self.data = dm.get_data()[-1]  # test
    ...
    def select_genre(data, filenames, genre):
        ...
        for i, filename in enumerate(filenames):
            if genre in filename:
                ...
                _dataset_index.append(data.dataset_index[i])

        _data = Data(input_sentences=_tokens, edu_breaks=_edu_breaks, golden_metric=_golden_metrics,
                     decoder_input=None, relation_label=None, parsing_breaks=None, dataset_index=_dataset_index)
    ...
    def by_genre(self):
        ...
        results = dict()
        for genre in genres:
            cd, _ = self.select_genre(self.data, self.filenames, genre)
